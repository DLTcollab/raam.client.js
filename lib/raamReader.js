...
const { digest, isTrits, intToTrytes } = require('./helpers');
...
class RAAMReader {
  ...
  subscribe(callback, {
    ...
    const doSubscribe = (i) => {
      const wrap = (error, index, message, skipped, nextRoot) => {
        ...
        callback(error, index, message, skipped, nextRoot);
      };
    ...
    for (let i = start; end != undefined ? i <= end : true; i++) {
      ...
      try {
        ...
        if (message == undefined) {
          if (end == undefined) {
            if (callback) {
              await callback(undefined, index, undefined, singleSkipped);
            }
            break;
          }
        } else {
        ...
        if (callback) {
          await callback(undefined, index, message, singleSkipped, nextRoot);
        }
      } catch (e) {
        if (callback) {
          await callback(e, index, undefined);
        }
  ...
  static subscribeIndex(channelRoot, index, callback, {
    serverURL, subscribeFollowing = false, channelPassword, messagePassword, height, security,
  } = {}) {
    if (serverURL) {
    ...
    const sub = zmq.subscribe(address, (bundle) => {
      try {
        const response = reader.processBundle(index, bundle, key, { height, security });
        try {
          const { message, nextRoot } = processMessage(response, channelRoot, index);
          callback(null, index, message, [], nextRoot);
          sub.unsubscribe();
        ...
        callback(null, index, null, [{ bundle, error }]);
      }
    });
