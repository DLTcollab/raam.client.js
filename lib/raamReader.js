...
const {
  digest, isTrits, intToTrytes, publicPassword,
} = require('./helpers');
...
class RAAMReader {
  /**
     ...
     */
  constructor(channelRoot, {
    iota, channelPassword, security,
  ...
  /**
     ...
     */
  async syncChannel({
    iota = this.iota, callback, messagePassword, messagePasswords,
  } = {}) {
    const result = await this.fetch({
      iota, callback, messagePassword, messagePasswords,
    });
  ...
  /**
     ...
     */
  async fetch({
    iota = this.iota, index, start, end, messagePassword, messagePasswords, callback,
  } = {}) {
    ...
    for (const { start: s, end: e } of intervals) {
      const {
        messages, errors, skipped: batchSkipped, branches,
      } = await RAAMReader.fetchMessages(
        iota, this.channelRoot,
        {
          start: s,
          end: e,
  ...
  /**
     ...
     */
  async fetchPublic({
    iota = this.iota, index, start, end, callback,
  } = {}) {
    ...
    return this.fetch({
      iota, index, start, end, messagePasswords, callback,
    });
  }
  ...
  /**
     ...
     */
  subscribe(callback, {
    serverURL, index, start, end, subscribeFollowing = false, messagePassword, messagePasswords,
  } = {}) {
    ({ start, end } = getRange(index, start, end));
    if (end && end < start) {
      return;
    ...
    const doSubscribe = (i) => {
      ...
      const sub = RAAMReader.subscribeIndex(
        this.channelRoot, i, wrap,
        {
          serverURL,
          channelPassword: this.channelPassword,
          messagePassword: pw,
  ...
  /**
     ...
     * @throws {Error} if the serverURL is not passed and hasn't been set already.
     */
  subscribePublic(callback, {
    serverURL, index, start, end, subscribeFollowing = false,
  } = {}) {
    if (this.channelPassword != undefined) {
      throw new Error(Errors.PUBLIC_NOT_ALLOWED);
    }
    ...
    return this.subscribe(callback, {
      serverURL, index, start, end, subscribeFollowing, messagePasswords,
    });
  }
  ...
  /**
     ...
     */
  static async fetchPublicMessages(iota, addresses, {
    callback, channelRoot, height, security,
  } = {}) {
    ...
    for (const address of addresses) {
      try {
        ...
        if (callback) {
          await callback(
            undefined, singleResult.index, singleResult.message, singleResult.skipped,
  ...
  static async fetchMessages(
    ...
    for (let i = start; end != undefined ? i <= end : true; i++) {
      ...
      try {
        ...
        const pw = messagePasswords == undefined || messagePasswords.length <= i ? messagePassword : messagePasswords[i];
        const { message, skipped: singleSkipped, nextRoot } = await RAAMReader.fetchSingle(iota, channelRoot, i, {
          channelPassword, messagePassword: pw, security, height,
        });
  ...
  /**
     ...
     * @fulfil {SingleResult}
     */
  static async fetchSingle(iota, channelRoot, index, {
    channelPassword, messagePassword, height, security,
  } = {}) {
    const { message: response, skipped } = await reader.getMessage(iota, channelRoot, index, {
      channelPassword, messagePassword, height, security,
    });
    if (!response) {
  ...
  /**
     ...
     * @fulfil {SingleResult}
     */
  static async fetchPublic(iota, address, {
    index, channelRoot, height, security,
  } = {}) {
  ...
  /**
     ...
     * @throws {Error} if the serverURL is not passed and hasn't been set already.
     */
  static subscribeIndex(channelRoot, index, callback, {
    serverURL, subscribeFollowing = false, channelPassword, messagePassword, height, security, origin,
  } = {}) {
    if (serverURL) {
    ...
    const result = origin || {
      index, callback, channelRoot, subscribeFollowing,
    };
    let sub;
      ...
      try {
        ...
        try {
          ...
          if (subscribeFollowing) {
            const newSub = this.subscribeIndex(
              channelRoot, index + 1, callback,
              {
  ...
  /**
     ...
     * @throws {Error} if the serverURL is not passed and hasn't been set already.
     */
  static subscribePublic(address, callback, {
    serverURL, subscribeFollowing = false, index, channelRoot, height, security, origin,
  } = {}) {
    if (serverURL) {
      zmq.setServerAddress(serverURL);
    }
      ...
      try {
        ...
        try {
          ...
          if (subscribeFollowing) {
            ...
            const newSub = this.subscribePublic(
              nextAddress, callback,
              {
...
function processMessage(response, channelRoot, index, skipped) {
  const {
    message, signature, authPathHashes, verifyingKey, nextRoot,
  } = response;
