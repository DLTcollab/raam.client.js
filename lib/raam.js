...
class RAAM extends RAAMReader {
  ...
  constructor(leafs, hashes, height, { iota, channelPassword } = {}) {
    super(
      channelRootOrThrow(hashes, height),
      {
  ...
  createPublicMessageTransfers(message, { index = this.cursor, tag = 'RAAM', nextRoot } = {}) {
    ...
    return this.createMessageTransfers(message, {
      index, tag, messagePassword, nextRoot,
    });
  }
  ...
  /**
     ...
     * - if a message was already found at this index.
     */
  createMessageTransfers(message, {
    index = this.cursor, tag = 'RAAM', messagePassword, nextRoot,
  } = {}) {
    ...
    return sender.createTransfers(
      this.channelRoot, message, signature, index, verifyingKey,
      authPathHashes, {
        tag, channelPassword: this.channelPassword, messagePassword, nextRoot,
      },
    );
  }
  ...
  /**
     ...
     */
  async publishMessageTransfers(transfers, {
    message, depth = 3, mwm = 14, iota = this.iota,
  }) {
  ...
  /**
     ...
     * - if channel password is set.
     */
  async publishPublic(message, {
    index = this.cursor, tag = 'RAAM', depth = 3, mwm = 14, iota = this.iota, nextRoot,
  } = {}) {
    const { transfers, message: payload } = this.createPublicMessageTransfers(message, { index, tag, nextRoot });
    return await this.publishMessageTransfers(transfers, {
      message: payload, depth, mwm, iota,
    });
  }
  ...
  async publish(message, {
    index = this.cursor, tag = 'RAAM', depth = 3, mwm = 14, iota = this.iota, messagePassword, nextRoot,
  } = {}) {
    const { transfers, message: payload } = this.createMessageTransfers(message, {
      index, tag, messagePassword, nextRoot,
    });
    return await this.publishMessageTransfers(transfers, {
      message: payload, depth, mwm, iota,
  ...
  static async fromSeed(seed, {
    ...
    const { leafs, hashes } = await merkle.createTree(s, height, {
      security, offset, progressCallback: callback, timeout,
    });
