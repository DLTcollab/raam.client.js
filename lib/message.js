...
const {
  intToTrytes, trytesToInt, concat, padTritsMultipleOf, padTrytesMultipleOf, alphabet,
} = require('./helpers');
...
function createTransfers(
  merkleRoot, message, sig, index,
  verifyingKey, authPathHashes, {
    tag, channelPassword, nextRoot, messagePassword,
  } = {},
) {
  if (message.length > Math.pow(27, MESSAGE_LENGTH_TRYTES)) {
    throw new Error(Errors.INVALID_LENGTH);
  }
  const indexTrytes = intToPaddedTrytes(index, INDEX_TRYTES);
  ...
  const lengthTrytes = intToPaddedTrytes(message.length, MESSAGE_LENGTH_TRYTES);
...
async function sendMessage(
  iota, merkleRoot, message, sig, index, verifyingKey, authPathHashes,
  {
    tag = 'RAAM', depth = 3, mwm = 14, channelPassword, nextRoot, messagePassword,
  } = {},
) {
  const { transfers, message: raamMessage } = createTransfers(
    merkleRoot, message, sig, index,
    verifyingKey, authPathHashes, {
      tag, channelPassword, nextRoot, messagePassword,
    },
...
async function getMessage(iota, merkleRoot, index, {
...
function processBundle(txs, key, { index, height, security } = {}) {
  ...
  const payloadTransactions = Math.ceil(payloadLength / MESSAGE_FRAGMENT_TRYTES);
  ...
  for (let i = payloadTransactions; i < txs.length; i++) {
    ...
    result.signature.set(
      remaining >= MESSAGE_FRAGMENT_TRYTES * 3 ? frag : frag.slice(0, remaining),
