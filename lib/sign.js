...
function createPrivateKey(seed, security = 2) {
  return signing.key(padTritsMultipleOf(
    Kerl.HASH_LENGTH, Kerl.HASH_LENGTH,
...
function getMessageDigest(trytes, fragments) {
  ...
  const messageDigest = new Int8Array(Math.ceil(fragments / Kerl.HASH_LENGTH) * Kerl.HASH_LENGTH);
