...
function prettify(message) {
  return {
    ...
    signature: `${converter.trytes(message.signature).slice(0, 81)}...`,
    message: message.message,
    authPathHashes: message.authPathHashes.map(h => converter.trytes(h)),
    verifyingKey: converter.trytes(message.verifyingKey),
    nextRoot: message.nextRoot ? converter.trytes(message.nextRoot) : undefined,
  };
}
...
function digest(message, index, authPathHashes, verifyingKey, nextRoot) {
  return converter.trytes(concat(
    converter.trits(message), converter.trits(intToTrytes(index)),
    verifyingKey, nextRoot || [], ...authPathHashes,
  ));
}
